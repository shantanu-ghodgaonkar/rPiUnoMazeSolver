...
img_1=cv.addWeighted(img_og,9,img_og,0,0) #Increasing contrast for ball detection
...
img=cv.merge((b,r*0,g*0)) #Merging RGB as BGR and eliminating G and B components
...
_,img = cv.threshold(img,12,255,cv.THRESH_BINARY) #Thresholding image to convert to BW
# plt.imshow(img,'gray')
# plt.show()
...
if detected_circles is not None: 
        ...
        for pt in detected_circles[0, :]: 
                ...
                cv.circle(img, (a1, b1), r1, (255, 255, 255), -1) #Circle centre coordinates a1,b1

##Shape detect End


kernel = np.ones((3,3),np.uint8) #Setting kernel for morphing
img = cv.morphologyEx(img, cv.MORPH_CLOSE, kernel,iterations=5) #Close morphing (Dilation followed by erosion eliminates black dots)
img = cv.morphologyEx(img, cv.MORPH_OPEN, kernel,iterations=5)  #Open morphing (Erosion followed by Dilation eliminates white dots)
plt.imshow(img,'gray') #Image before resize
plt.show()
img=cv.resize(img, (300,300)) #resizing image
_,img = cv.threshold(img,12,255,cv.THRESH_BINARY) #Thresholding image to convert to BW
kernel = np.ones((3,3),np.uint8) #Setting kernel for morphing
img = cv.morphologyEx(img, cv.MORPH_OPEN, kernel,iterations=2)  #Open morphing (Erosion followed by Dilation eliminates white dots)
plt.imshow(img,'gray')
plt.show()